...
/*
 ...
 3.2: Функция должна вернуть массив, состоящий из тех дочерних
  элементов следующим соседом которых является элемент с тегом P
...
function findAllPSiblings(where) {
    var arr = [],
        arrElement = [...where.querySelectorAll('*')];

    for (var i = 0; i < arrElement.length; i++) {
        if ( arrElement[i].tagName === 'P' ) {
            arr.push(arrElement[i-1]);
        }
    }
...
function collectDOMStat(root) {
    ...
    function deepF(root) { 
        for (let elem of root.childNodes) {
            if ( elem.nodeType === 1 ) {
                if ( elem.tagName in result.tags ) {
                    result.tags[elem.tagName]++;
                } else {
                    result.tags[elem.tagName] = 1;
                }
                if (elem.classList.length !== 0 ) {
                    for (var classes of elem.classList) {
                        if ( classes in result.classes ) {
    ...
    var observer = new MutationObserver(function(mutations) {
        ...
        for (let mut of mutations) {
            if (mut.removedNodes.length >= 1 ) {
                obj.type = 'remove';
                for (let elemRem of mut.removedNodes) {
                    obj.nodes.push(elemRem);
                }
            }
            if (mut.addedNodes.length >= 1 ) {
                obj.type = 'insert';
                for (let elemAdd of mut.addedNodes) {
                    obj.nodes.push(elemAdd);
                }
            }
        }
        fn(obj);
